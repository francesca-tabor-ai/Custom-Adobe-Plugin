 ...
 * Pattern references from: Adobe-Templatization/_scripts/photoshop/smart-object-batch.jsx
 *   - Layer access: doc.artLayers.getByName() (line 139-156)
 *   - Smart Object replacement via Action Manager (line 158-172)
 *   - Disclaimer validation: check text layer contents (line 175-196)
 */
...
import type { ApplyResult, PlaceResult, ValidationResult, ValidationItem } from "../../../shared/src/types/validation-result";
...
export class PhotoshopAdapter implements HostAdapter {
  ...
  async applySwatches(swatches: Swatch[]): Promise<ApplyResult> {
    ...
    // Use batchPlay to add colors to the Swatches panel
    const { action } = ps;
    ...
    for (const swatch of swatches) {
      try {
          ...
          [
            {
              ...
              new: {
                ...
                color: {
                  _obj: "RGBColor",
                  red: swatch.rgb.r,
                  grain: swatch.rgb.g, // Photoshop uses "grain" for green
                  blue: swatch.rgb.b,
                },
        ...
        // If colorSampler fails, set as foreground color as fallback
        try {
  ...
  async placeLogo(asset: LogoAsset): Promise<PlaceResult> {
    ...
    try {
      // Use batchPlay to place an embedded smart object
      const { action } = ps;
      await action.batchPlay(
        [
          {
  ...
  async validateColors(brandSwatches: Swatch[]): Promise<ValidationResult> {
    ...
    // Check all layers for fill colors and text colors
    function checkLayer(layer: any, path: string): void {
      try {
        // Check text layers for text color
        // Pattern from smart-object-batch.jsx:148 — layer.kind === LayerKind.TEXT
  ...
  async validateDisclaimerPresence(disclaimers: Disclaimer[]): Promise<ValidationResult> {
    ...
    // Collect all text from text layers
    // Pattern from smart-object-batch.jsx:175-196 — validateVariant checks DISCLAIMER_TEXT layer
